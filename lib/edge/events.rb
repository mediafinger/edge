module Edge
  class Events
    ...
    def self.create_event(attributes)
      ...
      post('/events.json', :body => { :event => attributes })
    end
...
if defined?(Rails)
  Edge::Events.tap do |config|
    ...
    config.base_uri 'http://events.dev/'                            if Rails.env == "development"      
    end
  end
end